...
function Dashboard() {
  ...
  const fetchData = async () => {
    setFetching(true);
    try {
      const statsRes = await apiInstance.get(`student/summary/${UserData()?.user_id}/`);
      setStats(statsRes.data[0]);

      const courseRes = await apiInstance.get(`student/course-list/${UserData()?.user_id}/`);
  ...
  const handleSearch = (event) => {
    ...
    if (query === "") {
      ...
      const filtered = courses.filter((c) => c.course.title.toLowerCase().includes(query));
      setCourses(filtered);
    }
  };
